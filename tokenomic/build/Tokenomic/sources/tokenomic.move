...
module seapad::tokenomic {
    ...
    struct TokenomicFund<phantom COIN> has store {
        ...
        vesting_fund_total: u128, //total of vesting fund, inited just one time, nerver change!
        vesting_fund: Coin<COIN>, //all locked fund
        vesting_fund_released: u128, //total released
        pie_percent: u128 //percent on pie
    }

    struct TokenomicPie<phantom COIN> has key, store{
        id: UID,
        tge_ms: u64, //TGE timestamp
        total_supply: u128, //total supply of coin's tokenomic, pre-set and nerver change!
        total_shares: u128, //total shared coin amount by funds
        total_shares_percent: u128, //total shared coin amount by funds
                             ...
                             _ctx: &mut TxContext)
    {
        ...
        pie.total_shares = pie.total_shares + fundAmt;
        pie.total_shares_percent = (pie.total_shares * (ONE_HUNDRED_PERCENT_SCALED as u128))/pie.total_supply;
        let pie_percent = fundAmt * (ONE_HUNDRED_PERCENT_SCALED as u128)/pie.total_supply;
    ...
    public fun addFund2<COIN>(_admin: &TAdminCap,
                             pie: &mut TokenomicPie<COIN>,
                             owner: address,
                             name: vector<u8>,
                             vesting_type: u8,
    ...
    public fun claim<COIN>(pie: &mut TokenomicPie<COIN>,
        ...
        let claimPercent = (cal_claim_percent<COIN>(fund, now_ms) as u128);

        assert!(claimPercent > 0, ERR_NO_COIN);

        let total_token_amt = (fund.vesting_fund_total * claimPercent)/(ONE_HUNDRED_PERCENT_SCALED as u128);
        let remain_token_val = total_token_amt - fund.vesting_fund_released;
        assert!(remain_token_val > 0, ERR_NO_MORE_COIN);

        transfer::public_transfer(coin::split<COIN>(&mut fund.vesting_fund, (remain_token_val as u64), ctx), senderAddr);
    ...
    public fun getPieTotalSupply<COIN>(pie: &TokenomicPie<COIN>): u128{
        pie.total_supply
    }

    public fun getPieTotalShare<COIN>(pie: &TokenomicPie<COIN>): u128{
        pie.total_shares
    }

    public fun getPieTotalSharePercent<COIN>(pie: &TokenomicPie<COIN>): u128{
    ...
    public fun getFundVestingAvailable<COIN>(pie: &TokenomicPie<COIN>, addr: address): u128{
        let share = table::borrow(&pie.shares, addr);
        (coin::value(&share.vesting_fund) as u128)
    }

    public fun getFundReleased<COIN>(pie: &TokenomicPie<COIN>, addr: address): u128{
        let share = table::borrow(&pie.shares, addr);
        share.vesting_fund_released
    }

    public fun getFundTotal<COIN>(pie: &TokenomicPie<COIN>, addr: address): u128{
        let share = table::borrow(&pie.shares, addr);
