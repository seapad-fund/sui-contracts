...
module seapad::project_custome {
    ...
    public fun push_orders<COIN, TOKEN>(_adminCap: &AdminCap,
                                        ...
                                        version: &mut Version) {
        checkVersion(version, VERSION);
        let (i, n) = (0, vector::length(&buyers));
        while (i < n) {
            let buyer = *vector::borrow(&buyers, i);
            let amount = *vector::borrow(&amounts, i);
            assert!(amount > 0, EInvalidAmount);
            buy(buyer, amount, project, sclock, kyc, version);
    ...
    fun buy<COIN, TOKEN>(
        ...
        if (!table::contains(&mut state.order_book, buyer_address)) {
            ...
            table::add(&mut state.order_book, buyer_address, newBuyOrder);
            state.participants = state.participants + 1;
        };

        let order = table::borrow_mut(&mut state.order_book, buyer_address);
        order.coin_amount = order.coin_amount + amount;
    ...
    fun swap_token(
        ...
        let swap_ratio_coin = (swap_ratio_coin as u128);
        let swap_ratio_token = (swap_ratio_token as u128);
        let decimal_ratio_coin = (math::pow(10, coin_decimals) as u128);
        let decimal_ratio_token = (math::pow(10, token_decimals) as u128);
    ...
    fun validate_buy<COIN, TOKEN>(project: &mut Project<COIN, TOKEN>, buyer_addr: address, now_ms: u64, kyc: &Kyc) {
        assert!(!project.require_kyc || hasKYC(buyer_addr, kyc), ENotKYC);
